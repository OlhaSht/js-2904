...
console.log(hasElem(['1', '2', '3', 'help'], 'help'));


//2
function check (arr, key = '5'){
  for (let i = 0; i < arr.length; i++) {
    const element = arr[i];
    if (element === key)
    return true;
...
const pets = ['1', '2', '3'];
console.log(pets.includes('5'));


// //3
function dogs (){
  const sameDogs = [1, 1, 2, 2, 3, 4, 5];
  for (const i of sameDogs) {
    if (sameDogs[i] === sameDogs[i+1]);
  }
  return true;
};
console.log()
